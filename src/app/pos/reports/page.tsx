...
const MOCK_TRANSACTIONS = [
  { id: 'TRX-001', date: '2026-06-28', customer: 'Siti Aminah', item: 'Kebaya Encim Klasik (KB-002)', amount: 200000, deposit: 100000, status: 'Active' },
  { id: 'TRX-002', date: '2026-06-27', customer: 'Budi Santoso', item: 'Kebaya Bludru Hitam (KB-004)', amount: 250000, deposit: 150000, status: 'Completed' },
  { id: 'TRX-003', date: '2026-06-25', customer: 'Rina Melati', item: 'Kebaya Pengantin Putih (KB-005)', amount: 800000, deposit: 500000, status: 'Completed' },
  { id: 'TRX-004', date: '2026-06-22', customer: 'Ayu Wandira', item: 'Kebaya Kutu Baru Modern (KB-001)', amount: 150000, deposit: 100000, status: 'Completed' },
];

export default function ReportsPage() {
  const totalRevenue = MOCK_TRANSACTIONS.reduce((sum, trx) => sum + trx.amount, 0);
  const activeDeposits = MOCK_TRANSACTIONS.filter(t => t.status === 'Active').reduce((sum, trx) => sum + trx.deposit, 0);
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          Ringkasan pendapatan dan deposit aktif.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 flex items-start justify-between">
          <div>
            ...
            <h2 className="text-3xl font-bold text-gray-900">
              {new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR' }).format(activeDeposits)}
            </h2>
          ...
          <table className="w-full text-left text-sm text-gray-500">
            <thead className="bg-gray-50 text-xs uppercase text-gray-700">
              <tr>
                ...
                <th scope="col" className="px-6 py-3">Biaya Sewa</th>
              ...
              {MOCK_TRANSACTIONS.map((trx) => (
                <tr key={trx.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-gray-500 whitespace-nowrap">
                    {new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR' }).format(trx.deposit)}
                  </td>
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                      trx.status === 'Active' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'
                    }`}>
                      {trx.status}
                    </span>
                  </td>
